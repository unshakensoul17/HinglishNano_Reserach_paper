...
\title{\textbf{\nano{}-V5: A 7.7\,MB Multi-Task Encoder with\\ Deployment-Grade Retrieval Fidelity for Code-Mixed Hindi--English}}
...
\begin{abstract}
\nano{}-V5 is a 7.77M-parameter transformer (7.7\,MB dynamic INT8, 9.2\,ms single-thread CPU $P_{50}$) that produces, in a single forward pass, a 384-dimensional retrieval embedding together with memory-gating, domain, temporal-scope, and named-entity predictions for code-mixed Hindi--English (Hinglish). On a held-out full-corpus retrieval task (300 queries against 1{,}245 candidate memories), \nano{} INT8 retrieves the correct memory within the top-10 for 99.7\% of queries---statistically indistinguishable from an \texttt{all-MiniLM-L6-v2} fine-tuned on the identical split (100.0\%; $\Delta=1/300$ queries, paired bootstrap CI $[-1.0,+0.0]$ points)---while using $2.9\times$ fewer parameters, a $2.8\times$ smaller INT8 footprint, and $1.9\times$ higher single-thread throughput ($P_{50}$ 9.2 vs.\ 17.0\,ms). The fine-tuned teacher retains a significant 4.0-point top-1 advantage (88.0 vs.\ 92.0 R@1, paired $p=0.023$), which we report openly; a budget-matched control further shows that a shorter fine-tuning schedule (3 epochs, 195 steps) reverses the ranking entirely, a baseline pitfall we argue is common in small-model evaluations. Zero-shot general-purpose embedders collapse at corpus scale (57--67 R@1), and a 278M Hinglish-domain encoder loses 24.3 R@1 points when the candidate pool grows from 20 to 1{,}245---a degradation predictable from its embedding margin ($+0.03$). We release the held-out test split, all benchmark scripts, and per-query results.
\end{abstract}
...
\item \textbf{Deployment-grade fidelity.} Full-corpus R@10 of 99.7\%, statistically indistinguishable from its budget-matched fine-tuned 22.7M teacher ($\Delta = 1/300$ queries), at $2.9\times$ fewer parameters and $1.9\times$ the throughput, plus four auxiliary heads at no additional forward pass.
...
Table~\ref{tab:budget} reports the control that this paper's claims rest on. At 3 epochs the fine-tuned teacher scores 79.0 R@1 / 81.3 HardR@1 (1-vs-20) and \emph{loses} to \nano{} by 7 points; at 30 epochs it scores 99.0 and wins by 7. Any comparison of a task-trained small model against a lightly fine-tuned large one---in either direction---is unsound without budget disclosure. We report all \nano{}-vs-teacher statistics against the 30-epoch model.

\begin{table}[t]
\centering\small
\caption{Budget-matched control (1-vs-20 protocol). Reversal of the ranking between 3 and 30 fine-tuning epochs motivates mandatory budget disclosure for baseline fine-tuning.}
\label{tab:budget}
\begin{tabular}{lrrrrr}
\toprule
System & Steps & R@1 & HardR@1 & hard margin & STS-B $\rho$ \\
\midrule
\nano{} V5 (ours, 30 ep) & 5{,}250 & 92.0 & 92.3 & $+0.175$ & 65.3 \\   % FIX 5: stray "\ " removed
...
\nano{}-V5 demonstrates that a sub-8\,MB multi-task encoder can hold deployment-grade retrieval fidelity---top-10 recall statistically tied with a budget-matched fine-tuned teacher $2.9\times$ its size---while paying a quantified, disclosed price in top-1 precision and general-purpose STS. Future work, in order of expected payoff: checkpoint selection by held-out retrieval metric rather than composite score; filtering identity-positive pairs from the contrastive loss; consuming both curated hard negatives; moving the chatter augmentation into the training split and extending it with evaluative and reactive utterances; trimming the embedding vocabulary; LinCE \texttt{ner\_hineng} evaluation; and store-scale testing beyond $10^4$ candidates.
